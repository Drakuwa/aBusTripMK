...
public class baraj_mapa extends Activity implements OpenStreetMapConstants {
                ...
                {   
                	...
                	if (c.moveToFirst())
                	{
                		do {          
                			...
                			items.add(new OpenStreetMapViewOverlayItem(" ", ime, stanica));
                			
                			//Log.d("xxx", "ID: " + id + "  ime=" + ime + " lat/lon=" + flon + " / " + flat);
    ...
    @Override
        ...
        pMenu.add(0, MENU_CLEAR_MAP, Menu.NONE, R.string.clear_map).setIcon(android.R.drawable.ic_menu_close_clear_cancel);
    ...
    public boolean onPrepareOptionsMenu(Menu menu) {
        ...
        public boolean onMenuItemSelected(int featureId, MenuItem item) {
                switch(item.getItemId()) {
                        ...
                        case MENU_CLEAR_MAP:
                        		...
                        		for (Iterator<OpenStreetMapViewOverlay> iter = mOsmv.getOverlays().iterator(); iter.hasNext();) {
                        			Object o = iter.next();
        ...
        protected Dialog onCreateDialog(int id) {
                ...
                case DIALOG_ABOUT_ID:
                        return new AlertDialog.Builder(baraj_mapa.this)
    ...
    private void createGpsDisabledAlert(){  
     ...
     private void draw(GeoPoint srcGeoPoint, GeoPoint destGeoPoint, GeoPoint final_geo){
     ...
     private void create_dialog(final OpenStreetMapViewOverlayItem item){
    	 AlertDialog.Builder builder = new AlertDialog.Builder(this);  
       	 builder.setMessage("Кликнавте на: " + item.mDescription + ", Дали сакате да стигнете до таму?")
       	 	  .setIcon(R.drawable.icon)
              .setTitle(R.string.app_name)
       	      .setCancelable(false)  
	...
	private void DrawPath(GeoPoint src, GeoPoint dest, int color, OpenStreetMapView mMapView01)
	{
		
		URL url = null;
		try {
			url = new URL("http://www.yournavigation.org/gosmore.php?" + "flat=" + (src.getLatitudeE6() / 1000000.0)
					+ "&" + "flon=" + (src.getLongitudeE6() / 1000000.0) + "&" + "tlat="
					+ (dest.getLatitudeE6() / 1000000.0) + "&" + "tlon=" + (dest.getLongitudeE6() / 1000000.0) + "&" + "v="
					+ "motorcar" + "&" + "fast=1&layer=mapnik");
		} catch (MalformedURLException e) {
			//
		}
		try {
			URLConnection conn = url.openConnection();
			String userAgent = HttpUserAgentHelper.getUserAgent(getBaseContext());
			if (userAgent != null)
			...
			if (kml.indexOf("<coordinates>") != -1) {
				...
				//String test2 = coords.substring(coords.indexOf(',') + 1, coords.indexOf('\n'));

				//Log.d("xxx", "TEST-d=" + coords + " tralala =" + test + " NAJBITNO= " + test2);
				
				float lonRegularfirst = Float.parseFloat(coords.substring(0, coords.indexOf(',')));
				float latRegularfirst = Float.parseFloat(coords.substring(coords.indexOf(',') + 1, coords.indexOf('\n')).trim());
				
				//Log.d("xxx", "lonfirst=" + lonRegularfirst + " latfirst=" + latRegularfirst);
				
				GeoPoint startGP = new GeoPoint((int) (latRegularfirst * 1000000), (int)(lonRegularfirst * 1000000));
				...
				StringTokenizer tokenizer = new StringTokenizer(coords, "\n");
				   ...
				   while (tokenizer.hasMoreTokens()) {
					...
					if ((coord != null) && (coord.indexOf(',') != -1)) {
						...
						gp2 = new GeoPoint((int) (latRegular * 1000000), (int)(lonRegular * 1000000));
            	...
            	if (c.moveToFirst())
            	{
                	...
                	if (c2.moveToFirst())
                	{
                		do {
                			...
                			//Log.d("xxx", "ID na Korisni Linie: " + id_korisna_linija);
                			//int newmin=3000;
                        	...
                        	if (c3.moveToFirst())
                        	{
                        		do {
                        			...
                        			Cursor br_2=db.getRbr(id_korisna_stanica, id_korisna_linija);
                        			int rbr_currentdest=br_2.getInt(0);
                        			
                        			Cursor nas_2=db.getNasoka(id_korisna_stanica, id_korisna_linija);
                        			String nasoka_currentdest=nas_2.getString(0);
                        			
                        			//Log.d("xxx", "ID na Korisni Stanice: " + id_korisna_stanica);
                        			...
                        			if(current == null) {
                        				...
                        				Cursor c4 = db.getLocationStanici(id_korisna_stanica);
                        				...
                        				if (c4.moveToFirst())
                        				{
                        					...
                        					int rastojanie = currentG.distanceTo(stanica);
                        					
                        					if(rbr_finaldest>rbr_currentdest&&nasoka_currentdest.equalsIgnoreCase("A")){
                        						if (min>rastojanie){
                        							min = rastojanie;
                        							g = new GeoPoint(glon, glat);
                        							d = true;
                        							//Log.d("xxx"," KOORDINATI NA G: " + glon +" / "+ glat);
                        						}
                        					}
                        					
                        					else if(rbr_finaldest<rbr_currentdest&&nasoka_currentdest.equalsIgnoreCase("B")){
                        						if (min>rastojanie){
                        							min = rastojanie;
                        							g = new GeoPoint(glon, glat);
                        							d = true;
                        					...
                        					if (min==rastojanie) {//najdena e linija shto pominuva na ista stanica
                        						b=true;
                        						//Log.d("xxx", " VNATRE VO B rastojanie: " +rastojanie+ "  Konachan minimum = " + min);
                        						}
                        					//Log.d("xxx", " RASTOJANIE do sekoja stanica: " +rastojanie+ "  Nov minimum:  " + newmin + "  Konachan minimum = " + min);
                        				}
                        			} while (c3.moveToNext());
                        		}
                        	
                        		if(d){
                        			//Log.d("xxx", "VLEZENO E VO D!!! - za najden pomal minimum...");
                        			korisni_linii.clear();
                        			...
                        			//Log.d("xxx", "VLEZENO E VO B!!! - za najden ist minimum...");
                        			Cursor cX = db.getLinija(id_korisna_linija);
                        			String ime_linija = cX.getString(0);
                        			korisni_linii.add(ime_linija);
                        			b=false;
	        ...
	        public void onPostExecute(ArrayList<String> izlezni) {
	            ...
	            if(min<2000){   
              		Location current = mLocationOverlay.getLastFix();
              			...
              			//Log.d("DDD", "Geopointi shto se isprakjav: "+srcGeoPoint.toString() +" "+ destGeoPoint.toString()+" "+final_geo.toString());
              			draw(srcGeoPoint, destGeoPoint, final_geo);
              			mOsmv.setMapCenter(new GeoPoint(current));

              			final_dialog("Појдете до означената постојка со црвено и почекајте автобус со број: "+ korisni_linii);
                	}
              		else final_dialog("За жал нема постојка во Ваша близина што поминува од овде, побарајте нова станица...");
	        }
	    }
}
